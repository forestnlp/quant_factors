...
**下一步（按序歼灭）**
1. **多因子合成战役**（降换手战役收官后主线，结论12 成功分支）：`v_std_20`/`r_20d`/`vlm_turnover`/`auc_money_share` 逐个跑 `run_ls` 同法验证（每个都是现成管线，先 IC 已筛过的）→ 幸存者做行业中性 rank 均值合成 → 合成信号跑 `run_ls`，目标对冲口径 Sharpe 较单因子进一步提升且回撤更低；成功→合成因子=私有库第一个正式产品
