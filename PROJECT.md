...
**下一步（按序歼灭）**
1. **降换手战役·第三层：多空纸面对冲**（结论11 已判决导出，无第二选择）：`backtest.py` 增对冲口径——多 band(0.10,0.50] + 空 D9 端（q>0.90），市场贝塔归零验纯截面价差。及格线：对冲组合费后年化>0 且 Sharpe>0.5（纯截面口径，不再 vs 基准）。成功→信号入库转多因子合成；失败→降级为"仅规避用"转 alpha101 海选
