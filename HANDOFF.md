...
> 最后更新：2026-09-08（L4 挖掘回路 dig.py 上线实弹：首猎 candidate a_retail_chase_quiet；标签漏洞已堵）
...
3. ~~L4 换假设器~~ ✅ **2026-09-08 上线实弹（结论21）**：探枪全过（JSON mode + function-calling 原生可用）→ `dig.py` 回路建成，2 轮试猎 **1 过 1 拒零人工**：首猎 `a_retail_chase_quiet`（散户小单追逐×低换手，IS ICIR **0.907 建库最高**、IS 年化 +8.3%/0.52）入 candidate。**待办：①批量夜跑需用户批准（建议 --rounds 20 --patience 5）②人工重提被去重 bug 误杀的 a_low_vol_quiet（低波×低换手，IS ICIR 0.767）**；新候选升 product 一律走人工 WFO 预注册
