...
# 09-17 用户指令"加大深度思考时间"再放大 16000→32000（配合 sys_p 深度思考四步法）。
MAX_TOKENS = 32000
...
def propose(cols: list[str], exprs: dict, recap: list[dict],
    ...
    sys_p = (
        ...
        "\n【深度思考四步法——定稿前必须在思维链里逐步走完，宁慢勿浅】"
        "\n第一步·回归基础逻辑：不从既有因子出发，从市场的第一性原理出发——"
        "价格由谁的交易错误推动？哪类参与者（散户/机构/量化/产业资本）在什么"
        "约束下（考核期/流动性/仓位限制/认知偏差）必然做出可预期的错误动作？"
        "我的因子赚的到底是这笔什么钱。答不出'谁被迫把钱交给我'就是伪信号。"
        "\n第二步·哲学家思辨：对每个候选机制做三轮自省——①反题：给出这个信号"
        "会失效的最合理世界图景（若无反例说明思考不够深）；②归谬：若全市场都"
        "用这个因子，它赚钱的因果链在哪一环断裂，断裂前它能撑多久；③同构：这个"
        "机制与哪个已验证领域（行为金融/供需/委托代理）同构，同构是否意味着"
        "与现役某枪本质是同一笔钱（是则换皮，弃）。"
        "\n第三步·借鉴已有发现：显式对照在册活枪与官方骨架的机制图谱——新假设"
        "必须与最近的活枪在经济机制上拉开一条可陈述的界线（不是表达式不同，"
        "是赚钱的人不同/触发的情境不同）；并优先把已验证有效的结构骨架与未开垦"
        "原料杂交，而非凭空臆造。"
        "\n第四步·定稿前审计：原料是否用了未过度使用的矿、表达式是否编译得通、"
        "标签周期与兑现逻辑是否一致（几天赚的钱）、结论是否只是把死路换了层皮。"
        "四步走完后输出且只输出 JSON: "
