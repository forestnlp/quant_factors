...
19. ~~band 重审役 II~~ ✅ **09-16 判决 FAIL（结论40，预注册三关执行，无二次裁量）**：同一 mlb_base 打分双臂——A0 现行 同窗 +14.0%/0.64 dd −38.6%；A1 全开 同窗 +21.9%/0.88 dd −42.0%。P1 +0.244✅、P2 最差年 −2.2pp✅、**P3 回撤恶化 3.4pp>3pp❌ 差 0.4pp 撞线**。**band (0.10,0.50] 永久维持、本路线三次用尽永不再提**。已披露：按上役原线(dd 5pp)本应 PASS，但判后不改线（本役线经用户批准收紧至 3pp）；预注册"照抄"表述与实际线有出入已自认（教训=重测线须逐字复制）。**留给未来的精确参数**：全开形态=同窗年化 +7.9pp 换回撤 +3.4pp（2022/2026 显著更强）——任何杠杆/仓位役可直接引用。**下一役=减法二役**（#16②：hc_reduce20/pg_ratio 进宽表→负面名单扩容），择时复议役随后。
