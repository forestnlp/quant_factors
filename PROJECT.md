...
**下一步（按序歼灭）**
1. **合成二期（新假设新协议）**：结论13 判等权合成无增益；候选方向=三强池（auc+vlm+v_amt）或风险预算加权——须先立新协议（建议改滚动 WFO 验证，OOS 已消费不得回看）再打。当前产品=`v_amt_5_20` 单因子对冲
3. **财务三表补齐**（成长/质量因子原料，通道现成；当前 L3 结论全部来自量价/资金面，财务是增量空间最大的一块）
4. `update.py` 真实缺口实弹验证（次日新交易日）
5. L4 挖掘层——**开工前必须先过上面《七种失控形态对策表》逐条验收**；先手工挖几轮验证"假设→评估→回喂"闭环，再谈自动化
6. 择机：实盘化研究（融券券源/保证金/冲击成本）、2005~2019 历史补齐、`signals/` 生产层实装
