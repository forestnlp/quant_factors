...
> 最后更新：2026-09-15 上午（**band 头端敞口战役判决 FAIL（结论35）**：A1 全开增量 +0.11 不过 P2、2024 差 11.7pp 不过 P3——band 维持 (0.10,0.50] 原样、路线关闭；ml_lgbm 现行=同窗 +16.3%/0.72。原料直喂 ML 判死（ml_lgbm_raw 0.45<0.72），行业/解禁走 dig 过闸正门。日内下午段用户拍板**暂缓**（HANDOFF#15 条件触发）。**在跑**：ML 重跑吃 160 枪（日志 `data/derived/ml_combo_160.log`）、dig 健康（160 在册、夜产 20、13 支新矿枪）。详见 PROJECT.md 结论33/34/35）
...
14. **ML 重跑吃新枪（待打，机器活）**：账本一夜从 144→160（09-14 起新增 30 支、其中 13 支用 `ind_*/unl_*` 新矿，头枪 `a_unl_cold_value_no_st` IS 0.94 / `a_lead_slow_cheap_goodwill_cold` 0.83 已用新原料），但现行 ml_lgbm 信号是 144 支时生成的快照。**动作**=重跑 `python -m research.ml_combo`（~90 分钟后台）看 0.72 是否上台阶。**预期按结论31 校准**：同族弹药对 ML 数量免疫，真正的增量只能来自新矿家族成规模（现 13/160 太少）。
...
16. **事件三件套取数战役（09-15 用户点单："优先取 业绩预告+回购+增减持"）**：减法首战（结论36，+1.0pp）定性"方向对、弹药少"→ 用户拍板扩弹药库。**候选清单**：①业绩预告/快报（STK_PERFORMANCE_FORECAST/EXPRESS，暴雷预减=减法主弹药，已实证可取）②回购（STK_SHARE_REPURCHASE，待接力探针实测属性名）③股东/高管增减持（STK_SHAREHOLDERS_SHARE_CHANGE，待测）；顺带探针还测质押/股东人数/十大股东/北向/指数权重/申万行业指数（菜单全量回来再定二梯队）。**取数姿势**=照解禁战役套路：`fetch` 新任务+断点续跑+读回验收+挂 update 日更（公告类向前生长）；**用法**=先走负面名单（减法管线已验证，预告预减/暴雷票剔出买入带），正面事件（回购/增持）作 dig 新原料入宽表列。**排队机制**：探针接力进程守 min_agg_pm 回填释放通道（单通道纪律），数据探针结果落 `data/derived/probe_more_out.csv`。
