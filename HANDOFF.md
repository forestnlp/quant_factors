...
> 最后更新：2026-09-15 晚（**收编判决役在跑**：Top15 官方原值 163 片全史回填收官+合片验收满分（772 万行/15 列，边界值一致），`ml_combo --battle` 双臂同池判决运行中（mlb_base±官方Top15 × 现行band/全开 四格，日志 `data/derived/ml_battle_official.log`，对照基线同窗 +16.3%/0.72、RankIC 0.1149）。**缺口探针实锤（结论34 纪律，10 接口一次云端审计）**：Alpha191 原值 ✅188 列非全NaN（弹药库扩编，`fetch_alpha --lib 191` 榜对榜 163 片回填在跑，日志 `alpha191_board_backfill.log`）；STK_XR_XD 除权除息 ✅47 列（股息/分红域翻案可建）；分析师一致预期/宏观 shibor/CPI ❌判死（真缺口，候选 AKShare 通道）。**在跑**：battle、191 回填、dig 夜产（167 在册）。**明日**：①收 battle 四格数字判决归档；②191 回填收官后扩编榜（alpha_board 对表）选 Top 批进 ML 二判决；③减法二役（增减持/质押进宽表）。详见 PROJECT.md 结论37/38）
...
18. **Alpha101 收编战役（09-15 用户"广泛采纳收编，兼收并蓄"拍板，结论37）——进行中，冷启动先查这条**：①官方 101 支源码/公式已存档 `data/raw/jq/alpha_ref/official_src.txt`（云端 inspect.getsource 取回，权威含 A 股适配）；**A 腿本地复算已判死**（16 种算子含截面语义，复刻风险高），**B 腿官方原值直取进行中**：`fetch_alpha --step 1 --tag top15`（Top15=alpha_016/088/013/044/040/015/027/050/055/003/026/012/081/094/004，IS/OOS 同号且 |IS ICIR|≥0.35）163 片全史日频回填（日志 `data/derived/alpha_top15_backfill.log`，**断点续跑=重跑同命令**，通道单占用等 min_agg_pm 队列）。②回填完成验收（列齐/NaN/采样日与旧 5 日缓存对答案——官方值应完全一致）→ `ml_combo --extra` 机制接 top15 列（tag=official）重训判决：**及格参考=同窗 +16.3%/0.72 基线，预期赌注=真低血缘带来 RankIC 破 0.1149 平台**。③官方头部结构骨架写进 dig prompt（喂骨架不喂答案）**未做**。④下午段 5 抽月已 119 天全齐，build_min_pm 特征+预注册闸（#15）待打。
