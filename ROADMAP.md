...
搭建**本地大模型驱动、7×24 小时全自动闭环**的因子 Loop Engineering 系统：

1. **自动挖掘**：本地大模型批量产出差异化候选因子（月产千级）。
...
- [x] 切换本地模型 Qwen3.8-Flash，单轮闭环跑通（3 生成 → 2 个有效因子，RankICIR 0.21-0.27）。
- [ ] 统计模型常见错误（未来函数/非法算子如 `Rank(x,1)`/数据错位），固化基础 Prompt 约束。
...
| `research/llm_client.py` | 本地大模型 OpenAI 兼容客户端（自动剥离 reasoning） |
| `research/factor_miner.py` | 单轮挖掘闭环：Prompt→LLM→批量 Qlib 回测→结果落盘 |
