...
5. **P0 合成战役（挖掘批跑完后开打，预注册）**：现役+新过 WFO 的候选等权合成，判决线事前定死
6. **P1 分钟→日频特征（用户 2026-09-09 提级，先于回填）**：先取 1 年 5 分钟样本 → 板块内"首小时**量比**领先度"（用户强调量比优先于价格）→ 进 eval 流水线验 IC；附属：行业/概念季度快照的日频化留待试点有效后
