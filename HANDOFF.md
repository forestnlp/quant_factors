...
16. **择时复议役（09-15 用户改判"仓位减法可以考虑"，重开 09-08 终裁；预注册先行）**：**判决对象**=Top-10 产品之上叠仓位层（MA20 类规则降半仓，timing.py 现成）能否改善风险调整后持有体验。**尺子换对（09-08 役自曝的标尺错误必须修正）**：主判据=**防守指标**——全区间最大回撤改善 ≥8pp（-34.2%→≤-26% 同量级）**且** Calmar 提升 ≥0.15 **且** 2022/2024/2026 三个压力段至少 2 段回撤减半；**年化不设"必须同升"门槛**（牛市结构必杀防守层——上役就是这么误杀的），只加一条底线：年化下滑 ≤3pp。**WFO 纪律不变**：择时参数（均线窗/仓位档）只准用 2020-2022 IS 定，2023+ 为验证段只看一次。**候选臂**：T1 MA20+半仓（上役 IS 当选者直接复验，防重新调参）、T2 MA20+清仓（更极端档，仅当 T1 过线才加测）。**FAIL 处置**=择时路线维持关闭不再复议（改判机会已用掉）；**PASS 处置**=交用户终裁是否改产品形态（机器不自作主张）。信号源=产品现行 ml_lgbm 还是单枪，**开跑前与用户确认**。
