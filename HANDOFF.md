...
6. **P1 分钟→日频特征（2026-09-09 开工，管道全通，取数进行中）**：`fetch min_agg`（云端聚合 3m/30m/90m 三窗量价→日频，只取 09:30-11:00 省流量；51s/交易日）+ `build_min`（7 特征：m_gap3/30、m_pull30、m_trend90、m_vconc3、m_vdecay30、m_lead30=行业PIT内率先启动）。**2020-06 单月初检：量能特征 m_vdecay30 IS ICIR -0.663、m_vconc3 -0.604、m_pull30 +0.441 过线，与现役族相关≤0.22=真异族出现**（价格特征全弱，用户"量比优先"判断应验）。**⚠️ 进行中状态（09-09 12 时）**：5 抽月（2020/21/22/23/26 各 6 月）取数后台跑（log=data/derived/min_agg_fetch.log，断点续跑安全，完成约需再 1h）→ 齐后重建特征做**试点判决**（4 个 IS 月 ICIR 跨月一致且|ICIR|>0.25；组合证据用分层价差，抽月不能跑净值回测）→ 过线才立项**全期回填**（~1600 日×51s≈22h 夜间分批）走组合终裁+WFO。OOS 证据=2026-06（只看一次）
