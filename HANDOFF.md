...
> 最后更新：2026-09-08（择时战役判决 FAIL：MA20+半仓防守大胜但年化验收否决，候选留档；timing.py 上线）
...
2. ~~仓位择时战役~~ ⚠️ **2026-09-08 判决 FAIL（结论20，但含重要未兑现价值）**：IS 当选 MA20+半仓（IS 0.87>0.79），全区间年化 +13.2% < 基线 +13.8% → 预注册"年化必须同升"一票否决。**防守端实际大胜**：回撤 -34.2%→-22.1%、Sharpe 0.66→0.79、Calmar 0.40→0.60、2026 段 -9.4%→-4.2%。**标尺错误自曝**：牛市里防守层结构上不可能年化更高，线定死了必输；MA20 数字已消费，**禁止改线翻案**——采纳路径=另立战役+事前改用 Calmar/回撤验收+此后新数据前瞻验证。择时层=候选留档（timing.py 保留）
3. **L4 换假设器**：本地 Qwen 探枪（function-calling/结构化输出质量实测）→ 过关则 PROPOSE.md 直接当系统提示词上岗；随后壳试用（dsh headless 或 LangGraph）跑一夜无人值守，七形态对策表逐项验收
4. **轮换扩池**：WFO 选枪证明同族二换一增益仅 +0.03——轮换要值钱必须扩异族池（fin_cash_quality 正交原料、择时信号、后续挖掘产出）
5. 择机：2005~2019 回填、signals/ 层、股指期货/两融等合规做空与杠杆工具可行性调研
