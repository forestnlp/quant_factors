...
15. **日内下午段（用户 09-15 拍板"暂缓，先收主线"，条件触发才打）**：用户提"日K太笼统、要日内真实形态"。已交底：日内信息确有金（竞价 `auc_imb/auc_money_share` 是最硬独立信号、OOS ICIR −0.71），但 P1 开盘分钟战役两周期全灭（结论26）。**未开采区=下午段**（尾盘 30m、收盘集合竞价、午后量比）——机制与开盘段不同，非旧题重做。打法已设计好待触发：复用 min_agg 管道在 P1 留存 5 抽月重聚合下午段（几小时）→ 预注册抽月闸（**标签周期须事前写死**，尾盘/尾盘竞价先验 1~5 日、双标签同审）→ 过闸才全期回填（~30h）。**触发条件**=主线（dig 行业新矿曲线、合成三战、候选清洗）出现弹药瓶颈或空闲算力时。
