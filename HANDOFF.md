...
25. **AKShare 预期域战役（探针已 PASS，2026-09-20，详情=结论56）**：业绩预告/快报接口可用+PIT 列齐+深到 2020。**下一动作**=research/fetch.py 加 `ak_exp` 任务（按报告期分片、raw 存原始快照 jq 目录旁立 ak/ 子目录）→ 全史回填 2020~ → build 派生宽表列（预告类型/超预期幅度/快报兑现差）→ dig 自动吃。**红线**：①"最新公告日期"疑为修订日，首公告日不能确认前不得进 build（宁缺毋假）；②AKShare 只作只读取数通道，限速克制；③东财代码=6 位纯数字，进宽表前须转 .XSHE/.XSHG 后缀（用 names.csv 对码）。
